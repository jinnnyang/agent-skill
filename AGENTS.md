...
- `scripts/` 目录下的代码必须以**实际的程序逻辑**为主：文件操作、网络请求、进程管理、数据处理、系统调用等。
- **禁止过多的说明性文字输出**——脚本的职责是自动化任务，而非替代 `SKILL.md` 传达长篇操作步骤。应专注于执行逻辑，仅保留必要的辅助说明。
- **自检标准**：脚本是否主要通过代码执行了有实质意义的任务？虽然允许必要的辅助说明以增强执行过程的可读性，但应避免创建一个删除说明行后几乎没有逻辑核心的脚本。

> ⛔ **反模式**：整个脚本全是 `console.log("请执行步骤1... 步骤2... 步骤3...")`——这是在浪费 Token 和计算资源。
>
> ✅ **正确做法**：脚本直接用代码执行步骤 1、2、3，自动完成任务，并输出简洁的执行状态。
